...
<section class="content">
    ...
    <div class="row">
      ...
      <div class="container-fluid">
        ...
        <ul class="nav navbar-nav">
          ...
          <li><input type="text" id="askSearch" class="form-control" style="width:400px;margin:10px 0" placeholder="Search" required></li>
        </ul>
        <button class="btn btn-danger pull-right navbar-btn" data-target="#addpop2" data-toggle="modal">Ask Question</button>
      </div>
    </nav>

    <!--Ask Us questions search-->
    <div class="col-lg-12">
     <div id="askResult" style='height:auto;background:#ffffff;'></div>
    </div>
    <!--ask us question results-->
                            ...
                            <div class="col-lg-12">
                              {!! Form::open(['route' => ['asks.store'], 'method' => 'POST']) !!}
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="category" id="categoryp1">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="sub_category" id="subcategoryp1">
                                      <option value=""></option>
                                    </select>
                                </div>
                                <div class="form-group">
    ...
    @foreach($asks as $ask)
      <div class="col-lg-10 col-md-offset-1">
        <div class="box box-default">
                 ...
                 <div class="box-body">
                   <h3><a href="{{route('asks.questions.show', $ask->id)}}" style="color:black">{{$ask->question}}</a></h3>
                 <div>
 ...
 <script type="text/javascript">
       $('#categoryp1').on('change', function(e){
         console.log(e);
         ...
         $.get('http://testserver.adviceli.com/public/newGet?cat_id=' + cat_id, function(data){
           ...
           $.each(data, function(index, subcatObj){
             $('#subcategoryp1').append('<option value="'+subcatObj.id+'">'+subcatObj.name+'</option>')
           });
         });
       });
     </script>
    ...
    <script type="text/javascript">
      $('#askSearch').on('keyup',function(){
        value = $(this).val();
        if(value.length > 0){
          $.ajax({
            type : 'get',
            url : '{{URL::to('ask-us/search')}}',
            data : {'query':value},
